...
(function () {
    ...
    // ── Playlist tracks ──
    const TRACKS = [
        { file: 'musica/tema1.mp3', title: 'Tema 1' },
        { file: 'musica/tema2.mp3', title: 'Tema 2' },
        { file: 'musica/tema3.mp3', title: 'Tema 3' },
        { file: 'musica/tema4.mp3', title: 'Tema 4' },
        { file: 'musica/tema5.mp3', title: 'Tema 5' },
        { file: 'musica/tema6.mp3', title: 'Tema 6' },
        { file: 'musica/tema7.mp3', title: 'Tema 7' },
    ];
    ...
    function connectSource() {
        ...
        // Solución para modo CD Local (file://):
        // Los navegadores silencian el audio cargado convencionalmente si pasa por Web Audio API por políticas CORS.
        // Hacemos el "bypass" de esta tecnología y permitiremos que suenen directamente los altavoces de forma normal.
    ...
    function buildPlaylist() {
        ...
        TRACKS.forEach((track, i) => {
            ...
            let nameHTML = track.title;
            let parts = track.title.split(' - ');
            if (parts.length > 1) {
                let artist = parts[0];
                let songTitle = parts.slice(1).join(' - ');
                let encodedArtist = encodeURIComponent(artist.trim());
                nameHTML = `<a href="https://es.wikipedia.org/wiki/Especial:Buscar?search=${encodedArtist}" target="_blank" class="artist-link" title="Buscar a ${artist.trim()} en Wikipedia" onclick="event.stopPropagation()">${artist}</a> - ${songTitle}`;
            }

            li.innerHTML = `
                <span class="track-num">${String(i + 1).padStart(2, '0')}.</span>
                <span class="track-name">${nameHTML}</span>
    ...
    function updateTechInfo() {
        ...
        const sizes = [4532224, 3264512, 3665920, 3289088, 3444736, 3121152, 3401728, 3743744, 4687872, 4278272, 2811904, 4081664, 4282368, 4419584, 3450880, 3450880, 4685824, 4018176, 4288512, 0, 2719744, 3246080, 4175872, 4091904, 4628480, 3383296, 4335616, 4464640, 26122, 3887104, 4075520, 4358144, 5025792, 3590144, 3241984, 3411968, 2861056, 6146048, 3971072, 4171776, 3309568, 3643392, 3428352, 3332096, 2834432, 3778560, 3051520, 4665344, 4552704, 3237888, 4206592, 4628480, 2824192, 3254272, 3645440, 3366912, 3739648, 3162112, 3682304, 4411392, 3543040, 3371008, 4298752, 2572288, 3563520, 3428352, 2846720, 2953216, 3348480, 3471360, 2852864, 4202496, 3835904];
    ...
    function drawVisualizer() {
        ...
        if (!analyser) {
            // Visualizador estético simulado para grabaciones locales en CD
            visualizerCtx.clearRect(0, 0, w, h);
            for (let i = 0; i < barCount; i++) {
                let barHeight = audio.paused ? 1 : Math.random() * h;
                const segments = Math.ceil(barHeight / 2);
                for (let s = 0; s < segments; s++) {
                    const y = h - (s * 2) - 2;
                    const ratio = s / (h / 2);
                    let r = ratio < 0.5 ? 0 : (ratio < 0.8 ? Math.floor(225 * ((ratio - 0.5) / 0.3)) : 225);
                    let g = ratio < 0.8 ? 225 : Math.floor(225 * (1 - (ratio - 0.8) / 0.2));
                    let b = 0;
